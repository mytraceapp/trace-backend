...
## Response Shape Lock (Phase 8.1)

`responseShapeContract.js` in `server/validation/` locks the API response envelope with three exports: `normalizeResponseEnvelope()` fills missing optional fields with null and strips unknown keys, `validateResponseEnvelope()` checks mode-specific invariants (studios must not have activity_suggestion/sound_state, crisis must have null audio/ui/activity/sound, onboarding source must be onboarding_script, conversation audio_action only with music intent), and `deriveResponseMode()` returns one of studios/conversation/crisis/onboarding/system. `applyResponseShapeLock()` in `index.js` wraps both normalize+validate at every `/api/chat` exit point (model pipeline, studios intercept, insight, dedup cache, activity followup, error fallback), emitting `[RESPONSE_SHAPE]` log with requestId, ok, mode, issues_count, top_issues, response_source, has_audio_action, has_ui_action, has_client_patch. Adds `_shape_meta` to every response for frontend debugging. Phase 8.1 is LOG-ONLY — violations are detected but responses are never rewritten. `server/tools/shapeReport.js` CLI summarizes shape logs. No prompt, schema enforcement, model routing, or behavioral changes. Files: `responseShapeContract.js`, `shapeReport.js`, `index.js`.
