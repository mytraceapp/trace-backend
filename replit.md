...
A per-user, timezone-aware push notification system using OneSignal. Sends personalized check-in messages at three "verse times" in each user's local timezone: 9:47 AM (morning), 3:16 PM (afternoon), 8:28 PM (evening). Uses slot-keyed deduplication (`last_checkin_at` stores `YYYY-MM-DD-slot` like `2026-03-04-morning`) so each verse time fires independently. Messages are randomized from ~32 templates across three time-of-day buckets, personalized with user's first name. Scheduler runs via `setInterval` every 60 seconds in `server/index.js`.
